...
\begin{multline*}
    ...
    \hfill\bigwedge_{M, M' \in S} \left( M_\emph{prev} = M'_\emph{prev} \land M_\emph{author} = M'_\emph{author} \land \textsc{CheckSignature}(M) \right)
\end{multline*}
...
A message $M$ in our design can have one of two types; either it has the type account (denoted \var{ACCOUNT}) or it has the type byzantine acknowledgement (denoted \var{BYZ\_ACK}). The type is encoded within its payload and retrieved with $\msgType(M)$. The roles of these message types is specified in ????. The function $\acc$ is the function that retrieves the account fields encoded in $M_\emph{payload}$, the author identifier from $M_\emph{author}$ (i.e. $(\acc(M))_\emph{author} = M_\emph{author}$), and is only defined if $\msgType(M) = \var{ACCOUNT}$.

\subsection{Choices}
\todo{write about why we choose to only include necessary and relevant dependencies in transactions, and how this prompts the use of BYZ\_ACK messsages}
...
Where $\var{wellFormed}$ is a predicate that is true iff the message is syntactically well formed (see section \ref{sec:syntax-of-msgs}), and $\var{valid}(M)$ is a predicate that is true if further invariants hold for the message specified in section \ref{sec:invariants}.

\todo{add \texttt{MsgType}}
...
Let $L_\emph{before}$ be the state of the ledger before update $\deltacc{A}$ was performed. More formally, this can be described as
\[
...
Since we merge account states no matter if the corresponding authors exhibited byzantine behaviour, this means that double spending is not prevented/handled correctly: if a double spending happens, tokens get created out of thin air by the equivocator. This leads to byzantine processes being able to generate an arbitrary number of tokens. However fixing this issue is more involved: if we simply ignore messages that come from byzantine processes, correct replicas that received money from such byzantine replicas lose tokens, which might lead to a negative balance which is not allowed under normal circumstances. An economical design that handles double spending in some way, preventing correct replicas from losing tokens due to invalidated messages is required to fix this issue.
...
\subsection{Design of Test Repository}
...
\subsubsection{Git Repository as Ledger}
\begin{figure}[h!]
    \dirtree{%
    .1 refs/.
    .2 heads/.
...
\subsection{Syntax restrictions on Git Objects}
\begin{itemize}
    ...
    \item ... \todo{add restriction on message}
\end{itemize}
...
We generate repositories of with the number of messages varying from 300 to 900 with a step size of 300.
To measure the runtime of different parts of the code, we use the \verb|perf_counter| function from the \verb|time| module in the standard library of python \cite{pythonPerfCounterNs}. To make sure these timing calls don't skew the performance measurements significantly, the same benchmarks were run without the timing calls and the resulting total runtime was still within ??? of the total runtime calculated by the granular timing calls. \todo{do this once the benchmarks are fixed}

\subsubsection{Storage Bench: Comparison of storage methods}
We take the simulation described in section \ref{sec:base-repo} and use it to create two repositories with the number of messages being 1000, where the only difference between the two is how the messages are stored. In the first one, the account data is stored as commit messages as described in \ref{sec:syntax-of-msgs}. In the second one, the account data is stored as nested tree and blob objects, as described in \cite{delta-goc-ledger-heisch}. More concretely: we store the delta-account $A^\delta$ as a tree object that contains children with entry-names ``c'', ``d'', ``a'', and ``g'', representing the fields $A^\delta_\uparrow$, $A^\delta_\downarrow$, $A^\delta_\leftarrow$, and $A^\delta_\rightarrow$ respectively. GOCs are represented as blob objects which contain the bytes that represent the number they store (using the functions \verb|int.to_bytes| and \verb|int.from_bytes| in the standard library of python to encode and decode respectively). The entries ``c'' and ``d'' contain references to blob objects which store GOCs. The entries ``a'' and ``g'' contain a reference to further tree objects, which represent the mappings from author names to GOCs, i.e. each entry-name of such a tree object is an author identity and references a blob object that represents a GOC \todo{if necessary: add figure that shows example of this visually.}.
...
\todo{explain the different parts of the plot (what does each label/number mean?)}

NOTE: here we don't include the time it takes to check the signatures of the commits.

First of all, we see that the total time spent grows more than linearly.

We see that the recreation of the frontier is by far the most time consuming. The reason for this is that we use a subprocess call to the git-cli to implement the recreation of the frontier.
