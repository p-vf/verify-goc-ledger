...
We also want to ensure that participants don't make transactions with users that haven't acknowledged the forks present in the message graph, i.e. if there is a fork in the log, all the immediate dependencies know about this fork proof. An exclusion is made for fork acknowledgement messages, as by design, they cannot fulfil this criteria.
\begin{enumerate}
    \item [$F1$] \emph{(No messages after fork)} If $\msgType(M) = \var{ACCOUNT}$, then the following must hold: for any fork proof $P$ of the log of author $a$ with $P \subseteq K(M)$, there must be no message $M'$ with $\msgId(M') \in M_\emph{deps}$ and $M'_\emph{author} = a$.
    \item [$F2$] \emph{(Fork acknowledgement)} If $\msgType(M) = \var{ACCOUNT}$ then the following must hold: \\
    \begin{itemize}
        \item For any message $M' \in \{N \mid \msgId(N) \in M_\emph{deps} \vee \msgId(N) = M_\emph{prev}\}$ and fork proof $P \subseteq K(M)$, $P \subseteq K(M')$ must hold. \\
    \end{itemize}
    If $\msgType(M) = \var{FORK\_ACK}$ then there must exist a fork proof $P \subseteq K(M)$, such that $P \not\subseteq K(M')$ where $M'$ is the message with $\msgId(M') = M_\emph{prev}$.
\end{enumerate}
