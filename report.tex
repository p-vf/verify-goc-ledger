...
\begin{algorithmic}[1]
    ...
    \ForAll{$msgId \in M_\emph{deps}$} \Comment $M4$
        \State $\var{Author} \gets (C_\emph{msg}[msgId])_\emph{author}$
        ...
        \If{$\var{Author} = M_\emph{author}$} \label{alg:m4:check2}
            \State \Return false
        \EndIf
        \State \var{Authors}[\var{Author}] $\gets msgId$
    \EndFor
    \Statex \ldots \vphantom{I}
\end{algorithmic}
\label{alg:m4}
...
\begin{algorithmic}
    \Statex \ldots \vphantom{I}
    \If{not $\Call{VerifySignature}{M}$} \Comment $M5$
        \State \Return false
    \EndIf
    \Statex \ldots \vphantom{I}
\end{algorithmic}
...
For the following code snippets, we assume that the variables $A_\emph{old}$ and $\deltacc{A}$ have been computed as in Algorithm~\ref{alg:dpre}.
