...
To retrieve messages in a reverse topological order, we use the command \texttt{git log} with the flags \texttt{--reverse} and \texttt{--topo-order} and, using format placeholders, we retrieve the relevant fields. \todo{should I list the format placeholders?} The commits are cached in a dictionary.
...
We split the code into various non-overlapping sections that are timed and grouped more or less arbitrarily.
