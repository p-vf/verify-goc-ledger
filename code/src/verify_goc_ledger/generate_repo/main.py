...
class ValidRepoGeneratorV1:
    ...
    def generate(self):
        ...
        for i in range(self._num_commits):
            ...
            commit_give = add_delta_state_as_commit(give_act, repo, msg=give_msg, deps=repo.show_ref(f"refs/heads/{acker.id.decode()}/last"))
            commit_ack = add_delta_state_as_commit(ack_act, repo, msg=ack_msg, deps=repo.show_ref(f"refs/heads/{giver.id.decode()}/last"))
...
class InvalidRepoGeneratorGoc:
    ...
    def generate(self):
        ...
        for i in range(self._num_commits):
            ...
            ack_msg = f"{(acker.id.decode())} acked {amount} CHF from {(giver.id.decode())}, has acked {acker.acked[giver.id]} CHF"
            commit_give = add_delta_state_as_commit(give_act, repo, msg=give_msg, deps=repo.show_ref(f"refs/heads/{acker.id.decode()}/last"))
            commit_ack = add_delta_state_as_commit(ack_act, repo, msg=ack_msg, deps=repo.show_ref(f"refs/heads/{acker.id.decode()}/last"))
            validate_hash(commit_give, "commit_give")
            validate_hash(commit_ack, "commit_ack")
            print(give_msg)
            print(ack_msg)
