...
In this report we propose a design of the $\delta$-GOC-Ledger \cite{delta-goc-ledger-heisch} over a byzantine fault tolerant log, which is largely based upon the design of 2P-BFT-Log proposed by Lavoie \cite{2p-bft-log}. In doing so, we discovered the need for a new message type (see section \ref{sec:choices}). In this report, the term ``invariant'' is considered synonymous with ``property''.
...
\section{2P-BFT-Log}
2P-BFT-Log is a design that implements an append-only log data structure and behaves just like one as long as there are no forks. As soon as there is evidence of a fork in a log, any further messages of the corresponding author are ignored \cite{2p-bft-log}.
...
\begin{table}
    ...
    $M_\emph{payload}$ & String & Content of message, in our case representing either an account update or a byzantine acknowledgement (see section \ref{sec:syntax-of-msgs}) \\
    ...
    $M_\emph{date}$ & Timestamp & Date the message was created at \\
    \hline
    \end{tabular}
...
\begin{equation}
    M \rightsquigarrow M' \definedas \begin{cases}
        M = M'_\emph{prev} = \bot \\
        \msgId(M) = M'_\emph{prev} \\
        \msgId(M) \in M'_\emph{deps} \\
...
We define the strict causal history of message $M$ as the set of messages that happened before message $M$:
\[
...
The balance of an account $A$ is defined as: \[\balance(A) \definedas A_\uparrow + \left(\sum_{a \in A_{\leftarrow*}} A_\leftarrow[a]\right) - A_\downarrow - \left(\sum_{a \in A_{\rightarrow*}} A_\rightarrow[a]\right)\]
Additionally there is a merge operation, denoted $\sqcup$, with which accounts can be merged. This merge operation is only defined on accounts of the same author and returns the account that contains the maximal values of all the grow only counters in both of the accounts and adds all the grow-only counters that are only in one of the two accounts. Formally, for accounts $A$ and $B$, the merged account $C = A \sqcup B$ is defined as:
\begin{align*}
...
Furthermore, there is also a merge operation on a ledger and an account, also denoted $\sqcup$. The ledger and an account get merged by merging the account with the corresponding account in the ledger if the author is present and if not, adding the account to the ledger. Formally, if we have a ledger $L$ and account $A$, $L' = L \sqcup A$ is defined as:
...
\end{equation}
where the inner most left argument, denoted as the empty set, is the empty ledger, and we use the second definition of the $\sqcup$ operator, i.e. where the left argument is a ledger and the right argument an account.
...
\chapter{Design}
In this chapter we present the design of the $\delta$-GOC-Ledger over 2P-BFT-Log, including the choices that were made, the invariants on the design and the algorithm for verifying the invariants.

\section{Definitions}
First, we define some terms that are used in subsequent sections. We start with the signature check predicate, denoted $\textsc{Signed}$ of message $M$. $\textsc{Signed}(M)$ is true iff $M_\emph{signature}$ is a valid signature belonging to $M_\emph{author}$.
...
\[
    ...
    \bigwedge_{M, M' \in S} \left(
        \begin{aligned}
            ...
            \land \textsc{Signed}(M')
        \end{aligned}
    \right) \land \lvert S \rvert \ge 2
\]

For ease of writing, we define some functions related to the immediate dependencies of a message.
...
A message $M$ in our design can have one of two types; either it has the type account (denoted \var{ACCOUNT}) or it has the type byzantine acknowledgement (denoted \var{BYZ\_ACK}). The type is encoded within its payload and retrieved with $\msgType(M)$. The roles of these message types is specified in section \ref{sec:choices}. The $\acc$ is the function that, given a message $M$, retrieves its account fields encoded in $M_\emph{payload}$, the author identifier from $M_\emph{author}$ (i.e. $(\acc(M))_\emph{author} = M_\emph{author}$), and is only defined if $\msgType(M) = \var{ACCOUNT}$. How this function is defined varies in each implementation, as the encoding of the account is not defined in the conceptual design of this data structure.
...
There are multiple authors that we need to track of during verification. We combine this information in a frontier. A frontier is a map of author ids to a set of messages and is defined over a set of messages $\mathcal M$, written $F(\mathcal M)$. The reason we map author ids to sets of messages as opposed to single messages is because an author may fork their log, creating multiple maximal elements in their log which we need to keep track of.
\begin{equation}
    \label{eqn:frontier}
...
For specifying invariants related to byzantine behaviour, it is necessary to define what we consider byzantine behaviour. This is mostly based on the Blocklace paper by Almeida et al. \cite{blocklace-crdt}, but was adapted to our design.
...
Where $\var{valid}$ is a predicate that is true iff the message is syntactically well formed (see section \ref{sec:syntax-of-msgs}), and further invariants hold for the message specified in section \ref{sec:invariants}. It is important to note that our design of the predicate $\var{valid}$ does not follow the convention proposed in the design of the Blocklace paper that messages from known byzantine actors are ignored when computing the valid state of a CRDT, see section \ref{sec:invariants-delta-a} for justification.

\section{Invariants}
\label{sec:invariants}
In this section we introduce the invariants of our design. We start by introducing the invariants related to the account state. Then the modified invariants of the 2P-BFT-Log are listed and then, we introduce the invariants related to byzantine behaviour.

\subsection{Invariants on message \(M\) and corresponding delta account}
\label{sec:invariants-delta-a}
The following invariants only apply, if $\msgType(M) = \var{ACCOUNT}$. We decided that delta-accounts may update more than one counter. This was done to open up the possibility to safe space, as multiple updates can be put into one message, splitting the overhead of storing a message among the updates.
...
\end{equation}
where $H_\emph{acc} = \{M' \in \mathcal H_<(M) \mid \msgType(M') = \var{ACCOUNT} \land \var{valid}(M')\}$ is the set of all delta accounts in the strict causal history of $M$, and $\bigsqcup$ is the iterated operator that applies the merging operations to the account states, resulting in a ledger (See Equation \ref{eqn:bigsqcup}).

Since we merge account states no matter if the corresponding authors forked their log, this means that double spending is not prevented nor handled correctly: if a double spending happens, tokens get created out of thin air by the equivocator. This leads to byzantine processes being able to generate an arbitrary number of tokens. However fixing this issue is more involved: if we simply ignore messages that come from byzantine processes, correct replicas that received money from such byzantine replicas lose tokens, which might lead to a negative balance which is not allowed under normal circumstances. An economical design that handles double spending in some way, preventing correct replicas from losing tokens due to invalidated messages is required to fix this issue but is outside the scope of this project.
...
The invariant \var{d1} ensures that there are no unnecessary GOCs stored in $A^\delta$. An unnecessary GOC is a GOC that doesn't increase the corresponding GOC in $L_\emph{before}$ when merged with it. This means each GOC in $A^\delta$ must be strictly greater than the previous value in $L_\emph{before}$ if it is defined.
\begin{invariant}{\var{d1}}
    ...
    \end{itemize}
\end{invariant}
The invariant \var{d5} ensures that at least one of the fields $\deltacc A_\rightarrow$, $\deltacc A_\leftarrow$, $\deltacc A_\uparrow$, $\deltacc A_\downarrow$ is defined, except for the first account message.
...
\end{invariant}
The invariant \var{d2} ensures that accounts don't have negative balance. See section \ref{sec:neg-balance} for reasoning.
\begin{invariant}{\var{d2}}
    (non-negative balance): The constraint $\balance(L_\textit{after}[M_\emph{author}]) \ge 0$ holds.
\end{invariant}
The invariant \var{d3} ensures that all the tokens that were acknowledged where actually given to the account.
\begin{invariant}{\var{d3}}
    (valid acknowledgements) The constraint $\deltacc A_\leftarrow[x] \le (L_\textit{before}[x])_\rightarrow[M_\emph{author}]$ must hold for all $x \in \deltacc A_{\leftarrow*}$.
\end{invariant}
...
\begin{invariant}{\var{d6}}
    (relevant dependencies): all the immediate dependencies of $M$ are relevant, i.e. for all $M' \in \var{deps}(M)$ it holds that $M'_\textit{author} \in \acc(M)_{\leftarrow *}$.
\end{invariant}
...
Furthermore, we need to check that there is at least one immediate dependency for each author mentioned in the field $\acc(M)_\leftarrow$.
...
That there is at most one immediate dependency per author is covered by the invariant \var{M4}, which is introduced in section \ref{sec:invariants-2p-bft-log}.

\subsubsection{On Negative Balance}
\label{sec:neg-balance}
For this project, we assume that the predicate \var{valid} may depend on messages in the causal history from known equivocators to avoid negative balance in valid account messages, which is in contrast to the design described in the Blocklace paper \cite{blocklace-crdt}, where the valid predicate may only depend on messages in the causal history from authors that don't exhibit byzantine behaviour. From this follows that if the balance of an account is negative, this can only have been caused by either a) the author overspending tokens or b) there being a fork in the causal history of the same account, where the double spending happened. In case a) the message is invalid and in case b) the message can also be viewed as invalid, because it is guaranteed to be created by a byzantine process, which means no message $M$ with $\msgType(M) = \var{ACCOUNT}$ should ever reference it anyway. So in either case, a negative balance invalidates a message.


\subsection{Invariants on Message \(M\) inherited from 2P-BFT-Log}
\label{sec:invariants-2p-bft-log}
\begin{enumerate}
...
We also want to ensure that participants don't make transactions with authors that haven't acknowledged the byzantine behaviour present in the message graph. This means that if there is byzantine behaviour in the set of messages reachable from a message, all its immediate dependencies must know about this (see invariant \var{F2}). An exception is made for byzantine acknowledgement messages, as - by design - they cannot satisfy this criteria.
\begin{invariant}{\var{F2}}
    \emph{(byzantine acknowledgement before transaction)}:\\
    If $\msgType(M) = \var{ACCOUNT}$ then for any message $M' \in \var{deps}(M)$,
    $$\var{byz}(\mathcal H_<(M)) = \var{byz}(\mathcal H_\leq(M'))$$ must hold.
\end{invariant}

To ensure that byzantine acknowledgement messages only have necessary immediate dependencies (in the same sense as before), we check that if any of the messages in the immediate dependencies were to be removed, the set of byzantine authors would shrink (see invariant \var{F3}).
...
The invariant \var{F4} ensures that additionally, any message that is in the dependencies must be a minimal message that proves its author byzantine.
...
If we don't check \var{F4} this allows byzantine colluders to pollute the causal history with arbitrarily many messages after a fork in a log. To show this, we can take a look at an example; If $E$ is a byzantine node and $C$ its colluder, where $E$ has forked their log and created many messages after this on either end. A colluder can then include any of these messages in the causal history with a byzantine acknowledgement message. So long as this acknowledgement is the first message that acknowledges the byzantine behaviour of $E$, this message is labelled as valid by \var{F3}. If we look at \var{F4}, we see that such behaviour would not be allowed. $C$ would have to acknowledge the first message that proves $E$'s byzantine behaviour. However, colluders may still include arbitrarily many messages into their causal history after a fork by only including messages from one branch of the fork and not acknowledging another branch, or they may keep creating bogus identities that create invalid or concurrent messages which get included by correct replicas as evidence for byzantine behaviour. The latter is analogous to the attack proposed by Lavoie \cite{blocklace-attack}.
...
The function \textsc{VerifyMessage} takes as argument the message $M$ to be verified, the message cache $C_\emph{msg}$ and the set of already validated messages, $F_\emph{valid}$ and is listed in Algorithm~\ref{alg:verify-message}. First off, we check that $M$ is syntactically well formed, i.e. $\var{wellFormed}(M)$ is true if not, we return false.
For readability the rest of the function \textsc{VerifyMessage} was split up into multiple conditions which return false if violated. These conditions are listed in sections \ref{sec:m-invariant-checks} and \ref{sec:delta-invariant-checks}.

\begin{algorithm}[ht]
    \caption{Basic structure of the function verifying individual messages}
    \label{alg:verify-message}
\begin{algorithmic}
...
The summary is an accumulation of information necessary to verify the invariants \var{M7}, \var{F1}, \var{F2}, \var{F3} and the invariants related to the delta account state on message $M$. A summary $S$ contains the fields listed in Table \ref{tab:summary-fields}\footnote{The fields $S_\emph{account}$ and $S_\emph{received}$ represent the relevant parts of $L_\emph{before}$ in Equation~\ref{eqn:l-before-def}}, and is always defined with respect to a message $M$, we write $S = \textsc{CreateSummary}(M)$.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|p{3cm}|p{7cm}|}
        ...
        Frontier of messages $F(\mathcal H_<(M))$ as in Equation~\ref{eqn:frontier}. Needed to keep track of what messages $M_\emph{author}$ must have known about before creating $M$.
        \\ \hline
        $S_\emph{byzantine}$ &
        Set of String (author)&
        Set of authors that exhibit byzantine behaviour in $\mathcal H_<(M)$. Needed for the check that $M$ isn't an acknowledgement of tokens from a known byzantine author.
        \\ \hline
        $S_\emph{byz\_acked}$ &
        Set of String (author)&
        Set of byzantine authors in $\mathcal H_<(M)$ acknowledged by $M_\emph{author}$. Needed for the check that if $\msgType(M) = \var{BYZ\_ACK}$, it must only have new byzantine authors in its dependencies.
        \\ \hline
        $S_\emph{account}$ &
        Account &
        The result of merging all the valid account messages in $\mathcal H_<(M)$ that belong to $M_\emph{author}$. Needed for checking non-negative balance and that counter updates are not redundant.
        \\ \hline
        $S_\emph{received}$ &
        Map of String (author) to Number &
        Maps authors to the number of tokens that were given by them to $M_\emph{author}$. Needed for checking the validity of any acknowledgement in $M$ if $\msgType(M) = \var{ACCOUNT}$ and $\acc(M)_\leftarrow \neq \emptyset$.
        \\ \hline
    \end{tabular}
...
To create the summary (see Algorithm~\ref{alg:create-summary}), we first have to initialize the fields and then iterate over all the messages in $\mathcal H_<(M)$ in topological order with the condition that their signature is valid. If we focus on the second statement (line \ref{alg:create-summary:frontier}) and the following in the for-loop, we can see how the frontier is built up over the iterations. The goal is to store the maximal messages of each author in the frontier $\var{Summary}_\emph{frontier}$. If we assume that $L$ contains the maximal messages up to but not including $N$, we know that if $N_\emph{prev}$ is not in $L$, this means $N$ is concurrent to the messages in $L$, which means we need to add it to $L$. $L$ gets then stored back in $\var{Summary}_\emph{frontier}$, which means we build the frontier of $\mathcal H_<(M)$ incrementally. If the size of this resulting $L$ is greater than 1, we know the creator of $N$ must have forked their log, thus $N_\emph{author}$ is added to $\var{Summary}_\emph{byzantine}$.
...
\Function{CreateSummary}{message $M$, set of messages $F_\emph{valid}$, map $C_\emph{msg}$}
    ...
    \State $\var{Summary}_\emph{received} \gets \emptyset$ \Comment{Map of author to number}
...
The simple invariants were re-expressed in terms of $C_\emph{msg}$, $M$ and $F_\emph{valid}$ and listed in Table \ref{tab:invariant-code-msg}. Each of the simple conditions corresponds to an if statement that returns false if the condition is met.
...
\var{M6} & never, because message graph is a cryptographic hash-graph \\ \hline
...
\begin{algorithmic}[1]
    ...
    \ForAll{$\var{Author} \in \var{Authors}_*$}
        \If{$\msgType(M) = \var{ACCOUNT}$}
            \If{$\var{Author} \in \var{Summary}_\emph{byzantine} \lor \var{Author} \notin \var{Summary}_\emph{byzantine}$} \Comment{F1}
                \State \Return false
                \label{alg:line:f1}
            \EndIf
        \Else \Comment $\msgType(M) = \var{BYZ\_ACK}$
    ...
    \If{$\msgType(M) = \var{ACCOUNT}$} \Comment \var{F2}
        \If{$\var{Summary}_\emph{byzantine} \ne \var{Summary}_\emph{byz\_acked}$}
            \State \Return false
        \EndIf
    \EndIf
    \Statex \ldots \vphantom{I}
...
If $M$ is a byzantine acknowledgement but the author was either already acknowledged as byzantine, or not byzantine, then this message is invalid as well (see line \ref{alg:line:f3-prime}) because it violates \var{F3}.
...
$\deltacc{A}$ represents the newly added delta account and $A_\emph{new}$ is the state of the account after applying $\deltacc{A}$.
...
As before, many invariant checks are simple if-conditions, which are re-expressed in terms of $A_\emph{old}$, $\deltacc{A}$, $A_\emph{new}$, $C_\emph{msg}$ and $M$, and listed in Table~\ref{tab:invariant-code-delta}.
\begin{table}[ht]
    \centering
\begin{tabular}{|c|l|}
\hline
...
\begin{algorithmic}
    ...
    \ForAll{$\var{Author} \in \deltacc{A}_{\leftarrow*}$} \Comment $\delta3$
        \If{$\var{Author} \notin (\var{Summary}_\emph{received})_* \lor \var{Summary}_\emph{received}[\var{Author}] < \deltacc{A}_{\leftarrow}[\var{Author}]$}
            \State \Return false
        \EndIf
    \EndFor
    \Statex \ldots \vphantom{I}
...
Now instead of always recreating the summary from scratch, we implemented an optimization that uses a mapping from authors to a modified version of their last summaries to reuse the already computed data. Let $S = \textsc{CreateSummary}(M)$ where $M$ is a message, then we compute the modified summary $S'$ that gets added to the cache by adding $M$ to the summary in the same way that the individual messages in the loop of \textsc{CreateSummary} get added. This message is then part of the message frontier, i.e. $\msgId(M) \in S'_\emph{frontier}[M_\emph{author}]$.

The modified summaries in the summary cache can then be used to compute $\textsc{CreateSummary}(M')$, where $M'$ is a message, more efficiently, if the modified summary $S'$ in the cache corresponding to author $M'_\emph{author}$ has $S'_\emph{frontier}[M'_\emph{author}] = \{M'_\emph{prev}\}$. This is the case if the previously visited message of author $M'_\emph{author}$ corresponds to $M'_\emph{prev}$ and $M'_\emph{author}$ did not acknowledge a fork of their own.

Once this condition is true, we can compute $\textsc{CreateSummary}(M')$ by only traversing the messages that come after all of the messages in $S'_\emph{frontier}$. The command to retrieve these messages in git is \texttt{git log <m>\^{} \^{}<f1> \^{}<f2> ... \^{}<fn>}, where \texttt{<fi>} represent the message ids in $S'_\emph{frontier}$ and \texttt{<m>} represents $\msgId(M')$.  The impact of this optimization can be seen in the Section~\ref{sec:results:perf-baseline}.
...
There is no way to get the signer of a commit using a format placeholder in the Git CLI. We can only check that a signature is valid, but not that $M_\emph{author}$ is the creator of $M_\emph{signature}$ (see \texttt{\%G?} in Table~\ref{tab:git-format-placeholders}), which makes this check alone useless. To remedy this, we run the \texttt{git show} command for each commit that has a valid signature (based on the signature validity retrieved with the format placeholder \texttt{\%G?}) with the \texttt{--format=raw} option, which prints the raw commit info including the base64 encoding of the signature, from which we can extract the signer and check that it matches $M_\emph{author}$.
...
Concerning performance, there are two things we are interested in: how is the scaling behaviour of the running time as we increase the number of messages we verify? and how does the optimization mentioned in Section~\ref{sec:impl-verification-alg} affect the runtime?
...
\subsection{Generating the Benchmark Repository}
\label{sec:base-repo}
...
One important thing to note when using Windows Subsystem for Linux 2 (WSL2) as a virtual machine is that the performance is degraded significantly when accessing files from a mounted directory \cite{wsl2FilesystemPerf}. We run the benchmark with associated data in the home directory, where this issue doesn't arise.

\begin{table}
\centering
\begin{tabular}{|l|p{6cm}|}
    \hline
    OS & Windows Subsystem for Linux version 2.7.10.0 running Ubuntu 24.04.4 LTS with Microsoft Windows 11 Pro as host OS\\ \hline
...
The benchmarks were only run once, which explains some of the variance in the results.
...
The results of the performance benchmark done on the baseline repository are shown in figure \ref{fig:perf-result}. The sections corresponding to ``signature check'', ``commit retrieval'', ``summary creation'' and ``remaining'' are the total time spent respectively checking the signatures of commits, retrieving all the commits initially, creating the summary and the remaining time not measured by any of the other ones. Most of the remaining time will be taken up by checking the invariants given the summary data structure.
...
\section{Storage Bench: Comparison of storage methods}
\subsection{Methodology}
...
Additionally, we see the repositories without the extra dependencies are between about 78\% and 92\% of the size of their corresponding repository with the extra dependencies.
...
Considering the implementation of the algorithm, we think the creation of the summary can be improved by running the commit traversal in the same process as the verification algorithm, rather than calling the Git CLI through the subprocess module of python. This is possible, because at the time of creating a summary, we have all the data that is needed to traverse the commit graph in $C_\emph{msg}$.
